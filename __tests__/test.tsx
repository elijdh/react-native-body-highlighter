...
describe("Body Component", () => {
  ...
  describe("Per-Part Style Props", () => {
    it("should apply per-part fill style", () => {
      ...
      const paths = UNSAFE_getAllByProps({ id: "chest-mid" });
      expect(paths.length).toBeGreaterThan(0);
      // All chest-mid paths should have the custom fill color
      paths.forEach(path => {
  ...
  describe("Global Default Style Props", () => {
    it("should apply defaultFill when no per-part style is provided", () => {
      const simpleData = [{ slug: "chest-mid" }];
  ...
  describe("Mixed Usage", () => {
    it("should handle mixed styling approaches", () => {
      const mixedData = [
        ...
        { slug: "deltoids-front", intensity: 2 },
      ];
  ...
  describe("Back Body Parts", () => {
    it.each(["male", "female"] as const)(
      "should render lats, upper-back and abductors on the %s back",
      (gender) => {
        const backData = [
          { slug: "lats", color: "#ff0000" },
          { slug: "upper-back", color: "#00ff00" },
          { slug: "abductors", color: "#0000ff" },
        ];

        const { UNSAFE_getAllByProps } = render(
          <Body data={backData} side="back" gender={gender} />
        );
        backData.forEach(({ slug, color }) => {
          const paths = UNSAFE_getAllByProps({ id: slug });
          expect(paths.length).toBeGreaterThan(0);
          paths.forEach(path => {
